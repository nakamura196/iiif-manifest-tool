...
export async function GET(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    const manifestPublicUrl = buildManifestUrl(baseUrl, userId, collectionId, itemId, 3);
    ...
    // Use the combined id format for canvas sub-resource URLs for compatibility
    const combinedId = `${userId}_${collectionId}_${itemId}`;
    if (manifest.items) {
      manifest.items = manifest.items.map((canvas: IIIFManifest['items'][0] & { 'x-canvas-access'?: { isPublic?: boolean; allowedUsers?: string[]; allowedGroups?: string[] } }, index: number) => {
        ...
        if (canvas.id && process.env.S3_ENDPOINT && canvas.id.includes(process.env.S3_ENDPOINT)) {
          canvas.id = `${baseUrl}/api/iiif/3/${combinedId}/canvas/${canvasNumber}`;
        }
        ...
        if (canvas.items?.[0]) {
          ...
          if (annotationPage.id && process.env.S3_ENDPOINT && annotationPage.id.includes(process.env.S3_ENDPOINT)) {
            annotationPage.id = `${baseUrl}/api/iiif/3/${combinedId}/canvas/${canvasNumber}/page`;
          }
          ...
          if (annotationPage.items?.[0]) {
            ...
            if (annotation.id && process.env.S3_ENDPOINT && annotation.id.includes(process.env.S3_ENDPOINT)) {
              annotation.id = `${baseUrl}/api/iiif/3/${combinedId}/canvas/${canvasNumber}/annotation`;
            }

            if (annotation.target && process.env.S3_ENDPOINT && annotation.target.includes(process.env.S3_ENDPOINT)) {
              annotation.target = `${baseUrl}/api/iiif/3/${combinedId}/canvas/${canvasNumber}`;
            }
        ...
        if (manifest['x-geo-annotations'] && manifest['x-geo-annotations'][index]) {
          ...
          if (geoAnnotation.points && geoAnnotation.points.length > 0) {
            canvas.annotations = [
              {
                id: `${baseUrl}/api/iiif/3/${combinedId}/canvas/${canvasNumber}/annotationPage`,
                type: "AnnotationPage",
                items: [
                  {
                    id: `${baseUrl}/api/iiif/3/${combinedId}/canvas/${canvasNumber}/annotation/georef`,
                    type: "Annotation",
                    motivation: "georeferencing",
                    target: canvas.id || `${baseUrl}/api/iiif/3/${combinedId}/canvas/${canvasNumber}`,
                    body: {
                      id: `${baseUrl}/api/iiif/3/${combinedId}/canvas/${canvasNumber}/feature-collection`,
        ...
        return NextResponse.json(
          {
            ...
            authService: `${baseUrl}/api/iiif/auth/access/${combinedId}`
          },
          {
            status: 401,
            headers: {
