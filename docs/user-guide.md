...
#### コレクションレベル
- **公開**: 誰でもアクセス可能（Self Museumでも表示されます）
- **非公開**: ログインユーザーのみ（Self Museumでは表示されません）
...
💡 **ヒント**: Self Museumで自分のコレクションを公開する場合は、コレクション設定で「このコレクションを公開する」にチェックを入れることを忘れないでください。デフォルトでは公開設定になっていますが、過去に作成したコレクションは非公開になっている可能性があります。
...
⚠️ **重要**: Self Museumで表示されるのは**公開設定になっているコレクションとアイテムのみ**です。非公開のコレクションやアイテムは表示されません。コレクション設定で「このコレクションを公開する」にチェックを入れてください。
